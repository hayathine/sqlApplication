...
//		削除ボタンが押された場合の処理
		if(request.getParameter("deleteOrderItem") != null) {
			Integer deleteOrderItem =  Integer.parseInt(request.getParameter("deleteOrderItem"));
			sqlQuery.deleteCartItem(deleteOrderItem);
		}
//		数量変更ボタンが押された場合の処理
		if(request.getParameter("updateOrderItem") != null) {
			int updateNum = Integer.parseInt(request.getParameter("updateNum"));
			int updateOrderItem = Integer.parseInt(request.getParameter("updateOrderItem")); 
			sqlQuery.updateCartNum(updateOrderItem, updateNum,0);
		}
