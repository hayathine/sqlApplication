...
<body>
	...
	<%if(!itemList.isEmpty()){ %>
		...
		<tr>
			...
			<!-- 商品選択ボタンから注文ページへ遷移 -->
			<td><form action="OrderController" method="GET"><button type="submit" name="orderCode" value="<%=i+1 %>">新規注文</button></form></td>
		</tr>
	<% }} %>
		</table>
	<!--  注文中の商品の表示 -->
	...
	<h2>カート内商品</h2>
	<table>
	<tr><th>商品名</th><th>価格</th><th>注文数量</th><th>代金</th><th>日時</th></tr>
	...
	<tr>
		...
		<form action="MenuController" method="GET">
			<td>
				...
				<button type="submit" name="updateOrderId" value=<%=cartList.get(i).getOrderId()%>>数量変更</button>
			<td>
				<button type="submit" name="deleteOrderId" value="<%=cartList.get(i).getOrderId()%>">削除</button>
			</td>
		</form>
	</tr>
	<%}} %>
	...
	<form action="OrderLogController" method="GET" >
		<input type="submit" name="selectOrderLog" value="注文履歴一覧">
	</form>
</body>
</html>